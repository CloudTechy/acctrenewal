...
async function testRouterConnection(config) {
  ...
  try {
    ...
    // Test hotspot commands with better error handling
    console.log(`🌐 Testing hotspot functionality...`);
    
    // First, check if hotspot package is installed
    try {
      const packages = await api.write('/system/package/print');
      const hotspotPackage = packages.find(pkg => pkg.name === 'hotspot');
      
      if (!hotspotPackage) {
        console.log(`   ⚠️  Hotspot package not installed`);
        console.log(`   💡 Install hotspot package: /system package enable hotspot`);
      } else if (hotspotPackage.disabled === 'true') {
        console.log(`   ⚠️  Hotspot package is disabled`);
        console.log(`   💡 Enable hotspot package: /system package enable hotspot`);
      } else {
        console.log(`   ✅ Hotspot package is installed and enabled`);
        
        // Now try to check hotspot configuration
        try {
          // Use a more specific command that's less likely to cause issues
          const interfaces = await api.write('/interface/print');
          console.log(`   📡 Network interfaces: ${interfaces.length}`);
          
          // Try to get hotspot servers with better error handling
          let hotspotServers = [];
          try {
            hotspotServers = await api.write('/ip/hotspot/print');
            console.log(`   🌐 Hotspot servers: ${hotspotServers.length}`);
          } catch (hotspotError) {
            if (hotspotError.errno === 'UNKNOWNREPLY' || hotspotError.message.includes('!empty')) {
              console.log(`   🌐 Hotspot servers: 0 (not configured yet)`);
            } else {
              throw hotspotError; // Re-throw if it's a different error
            }
          }
          
          if (hotspotServers.length > 0) {
            // Only check users if hotspot servers exist
            try {
              const activeUsers = await api.write('/ip/hotspot/active/print');
              console.log(`   👥 Active users: ${activeUsers.length}`);
            } catch (userError) {
              if (userError.errno === 'UNKNOWNREPLY') {
                console.log(`   👥 Active users: 0`);
              } else {
                console.log(`   ⚠️  Could not get active users: ${userError.message}`);
              }
            }
            ...
            try {
              ...
              if (userError.errno === 'UNKNOWNREPLY') {
                console.log(`   📋 Total users: 0`);
              } else {
                console.log(`   ⚠️  Could not get user list: ${userError.message}`);
              }
            }
          } else {
            console.log(`   💡 Router is ready for hotspot configuration!`);
          }
          
        } catch (interfaceError) {
          console.log(`   ⚠️  Could not check network configuration: ${interfaceError.message}`);
        }
      }
      
    } catch (packageError) {
      console.log(`   ⚠️  Could not check hotspot package: ${packageError.message}`);
    }
