...
type Row = { label: string; value: string; valueClass?: string };
...
export default function AccountDetailsPage() {
  ...
  const [accountStatusItems, setAccountStatusItems] = useState<Row[]>([]);
  const [currentPlanItems, setCurrentPlanItems] = useState<Row[]>([]);
  const [personalDetailsItems, setPersonalDetailsItems] = useState<Row[]>([]);
  const [usageDetailsItems, setUsageDetailsItems] = useState<Row[]>([]);
    ...
    async function load() {
      try {
        ...
        const statusRows: Row[] = [
          { label: 'status', value: (data.status as string) || 'Active', valueClass: 'text-[#19b76f] bg-[#12423a] border border-[#28504f] px-3 py-1 rounded-lg' },
          { label: 'Username', value: (data.username as string) || (data.email as string) || '-', valueClass: 'text-white/90' },
          { label: 'Expiry Date', value: (data.expiry_date as string) || '-', valueClass: 'text-[#19b76f]' },
          { label: 'Days to Expiry', value: data.days_to_expiry ? `${data.days_to_expiry} days` : '-', valueClass: 'text-[#eb5345]' },
        ];

        const plan = (data.plan as Record<string, unknown>) || {};
        const planRows: Row[] = [
          { label: 'Plan Name', value: (plan.name as string) || '-', valueClass: 'text-[#19b76f] bg-[#12423a] border border-[#28504f] px-3 py-1 rounded-lg' },
          { label: 'Monthly Price', value: (plan.price as string) || '-', valueClass: 'text-white/90' },
          { label: 'Validity', value: (plan.validity as string) || '-', valueClass: 'text-[#19b76f]' },
        ];

        const personal = (data.personal as Record<string, unknown>) || {};
        const personalRows: Row[] = [
          { label: 'Name', value: (personal.name as string) || '-', valueClass: 'text-white/90' },
          { label: 'Email', value: (personal.email as string) || '-', valueClass: 'text-white/90' },
          { label: 'Phone', value: (personal.phone as string) || 'N/A', valueClass: 'text-[#19b76f]' },
        ];

        const usage = (data.usage as Record<string, unknown>) || {};
        const usageRows: Row[] = [
          { label: 'Download Used', value: (usage.download as string) || '-', valueClass: 'text-[#19b76f]' },
          { label: 'Upload used', value: (usage.upload as string) || '-', valueClass: 'text-[#19b76f]' },
          { label: 'Total used', value: (usage.total as string) || '-', valueClass: 'text-[#19b76f]' },
        ];
        ...
        // finished loading; no local loading UI here
      }
    }
