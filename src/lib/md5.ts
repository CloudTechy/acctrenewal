...
/*
 * External interface - Main functions to export
 */
...
/**
 * MikroTik CHAP authentication helper
 * Computes CHAP response: MD5(chap-id + password + chap-challenge)
 */
export function computeChapResponse(chapId: string, password: string, chapChallenge: string): string {
  const chapInput = chapId + password + chapChallenge;
  return hexMD5(chapInput);
} 